...
> **H5** (dynamic `.so` integrity/signature gate — genuinely unimplemented) and
...
| H5 | High | Core & plugin loader | Dynamic `.so` plugins loaded with zero integrity/authenticity verification | `crates/aegis-core/src/loader.rs` | **Open** — no signature/hash/ownership gate (doc comment only) |
...
**Status: OPEN — not remediated.** This remains the single most significant unaddressed finding. `crates/aegis-core/src/loader.rs` still contains no signature/hash/ownership check (only the doc comment). *Related* loader hardening did land and partially reduces blast radius: enablement is now checked before the library is opened (closing H6), the entrypoint and constructors are wrapped in `catch_unwind` (M8), the registration is freed in the plugin's own allocator (H7), and a `.so` whose reported metadata name does not match its declared name is rejected — but none of these is an *integrity* gate. Tracked as ARCHITECTURE.md ADR #15.
