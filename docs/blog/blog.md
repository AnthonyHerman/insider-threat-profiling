...
Synthetic numbers test the *model*. They do not test the *system* — the part where a server and an agent are separate processes, talking over a real TLS socket, on a machine that is not your laptop. So we put it on one: a fresh Linode VM (Ubuntu 24.04), release binaries copied over, no Rust toolchain on the host, driven by `scripts/integration_demo.sh`. One `aegisd` binary came up with no database to install and no files to lay down beside it, minted an enrollment token, and the agent enrolled over pinned mutual TLS. We then fed it agent-like input — metronomic, pasted, no corrections — and the *server's* detector, scoring telemetry that had crossed the wire from another process, returned the verdict that matters: **`agent`, confidence 0.855**, with reasons `uncorrelated-flat-throughput`, `gap-non-autocorrelation`, `whole-line-injection`, `constant-think-time`, and the scorer raised a `critical` alert off the back of it. The architecture's central bet — that the network is just another event producer feeding the same feature-free kernel — held up end to end on real hardware.

The more useful part is what broke. The live run caught **two real bugs that every in-process test had sailed past**, because those tests run the whole pipeline over the in-memory bus and never actually enroll, authenticate, and forward across a socket. First, the forwarder authenticated its telemetry session with the *wrong identity* — it sent the local config's `agent_id` instead of the UUID the server had assigned at enrollment — so the agent enrolled fine and then silently never reported, every session rejected as "unknown agent." Second, pipe-mode telemetry was being dropped by a hardening guard that discards keystrokes for any session without a `SessionStart`: the pipe path emitted `SessionEnd` but leaned on another plugin's login event for the start, and the two `session_id`s lined up on the dev box but *not* under a remote SSH login — so no detection fired at all. Both are now fixed, and both are now covered by the integration script. The lesson is old but worth re-learning out loud: for a client/server tool, passing unit tests tells you your logic is right; only running it on a real box, across a real wire, tells you it's actually wired up.

## What's next / try it

The honest forward look maps onto the gap between an implemented prototype and a trustworthy product. The end-to-end path is wired and now runs on real hardware — capture (`plugin-tty`), forwarder, TLS ingest, store, detection, scoring — but it has been driven by one integration host with synthetic input, not by production traffic. So the work ahead is hardening and validation, not construction: exercising the live capture-to-verdict loop on real *interactive* sessions instead of synthetic streams; red-teaming the privileged install/guardian/immutable lifecycle on real hosts; load-testing ingest and the forwarder's spill-and-replay durability at more than one connected agent; and — the one that actually moves the accuracy claim — an IRB-approved field study that replaces the synthetic sessions with timing from instrumented humans and real agents.
